...
class RegionUrlProvider {
  ...
  async getNextBestRegionUrl(abortSignal) {
    if (!this.isCloud()) {
  ...
  async fetchRegionSettings(signal) {
    const regionSettingsResponse = await fetch("".concat(getCloudConfigUrl(this.serverUrl), "/regions"), {
      headers: {
    ...
    if (regionSettingsResponse.ok) {
      const regionSettings = await regionSettingsResponse.json();
...
class RTCEngine extends eventsExports.EventEmitter {
  ...
  async restartConnection(regionUrl) {
    var _a, _b, _c, _d, _e;
    try {
      ...
      try {
        ...
        joinResponse = await this.join(regionUrl !== null && regionUrl !== void 0 ? regionUrl : this.url, this.token, this.signalOpts);
      } catch (e) {
      ...
      const nextRegionUrl = await ((_d = this.regionUrlProvider) === null || _d === void 0 ? void 0 : _d.getNextBestRegionUrl());
      if (nextRegionUrl) {
  ...
  async waitForPCInitialConnection(timeout, abortController) {
    if (this.pcState === PCState.Connected) {
      return;
    }
...
class Room extends eventsExports.EventEmitter {
  ...
  constructor(options) {
    ...
    this.connect = async (url, token, opts) => {
      ...
      const connectFn = async (resolve, reject, regionUrl) => {
        var _a;
        ...
        try {
          await this.attemptConnection(regionUrl !== null && regionUrl !== void 0 ? regionUrl : url, token, opts, this.abortController);
          ...
          if (isCloud(new URL(url)) && e instanceof ConnectionError && e.reason !== 3 /* ConnectionErrorReason.Cancelled */) {
            let nextUrl = null;
            try {
              nextUrl = await urlProvider.getNextBestRegionUrl((_a = this.abortController) === null || _a === void 0 ? void 0 : _a.signal);
            } catch (error) {
    ...
    this.connectSignal = async (url, token, engine, connectOptions, roomOptions, abortController) => {
      const joinResponse = await engine.join(url, token, {
    ...
    this.attemptConnection = async (url, token, opts, abortController) => {
      var _a;
      ...
      try {
        const joinResponse = await this.connectSignal(url, token, this.engine, this.connOptions, this.options, abortController);
        this.applyJoinResponse(joinResponse);
  ...
  maybeCreateEngine() {
    ...
    }).on(EngineEvent.Restarting, this.handleRestarting).on(EngineEvent.Restarted, this.handleRestarted).on(EngineEvent.DCBufferStatusChanged, (status, kind) => {
